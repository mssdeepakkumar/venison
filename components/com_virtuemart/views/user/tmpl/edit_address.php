...
<form method="post" id="userForm" name="userForm" class="frm_ui_fr_shp form-validate" action="<?php echo JRoute::_('index.php?option=com_virtuemart&view=user',$this->useXHTML,$this->useSSL) ?>" >
<fieldset>
	<h2 class="cmn_hd_cart"><?php
	...
	<div class="control-buttons">
		...
		if (VmConfig::get ('oncheckout_show_register', 1) && $this->userDetails->JUser->id == 0 && $this->address_type == 'BT' and $rview == 'cart') {
			?>
			<button name="register" class="btn-custm <?php echo $buttonclass ?>" type="submit" onclick="javascript:return myValidator(userForm,true);"
			...
			<?php if (!VmConfig::get ('oncheckout_only_registered', 0)) { ?>
				<button name="save" class="btn-custm <?php echo $buttonclass ?>" title="<?php echo vmText::_ ('COM_VIRTUEMART_CHECKOUT_AS_GUEST'); ?>" type="submit"
						onclick="javascript:return myValidator(userForm, false);"><?php echo vmText::_ ('COM_VIRTUEMART_CHECKOUT_AS_GUEST'); ?></button>
				<?php } ?>
			<button class="btn-custm default" type="reset"
			...
			?>
			<button class="btn-custm <?php echo $buttonclass ?>" type="submit"
